...
interface Dia { fecha: string; ventas: number; otros_ingresos: number; compras: number; gastos: number; otros_egresos: number; entradas: number; salidas: number; neto: number; acumulado: number }
      ...
      {t && (
        <div className="tiles" style={{ marginTop: 0 }}>
          ...
          <div className="tile"><div className="t">Salidas</div><div className="v dif-mal">{money(t.salidas)}</div>
            <div className="s">Compras {money(t.compras)} · gastos {money(t.gastos)}{t.otros_egresos ? ` · otras ${money(t.otros_egresos)}` : ''}</div></div>
      ...
      <div className="tabla-wrap">
        <table className="tabla" style={{ minWidth: 860 }}>
          <thead><tr>
            ...
            <th className="num">Gastos</th><th className="num">Otras salidas</th><th className="num">Neto</th><th className="num">Acumulado</th>
          </tr></thead>
          <tbody>
            {data?.dias.map((d) => (
              <tr key={d.fecha} style={{ cursor: 'default' }}>
                ...
                <td className="num">{money(d.otros_egresos)}</td>
                <td className={'num ' + (d.neto >= 0 ? 'dif-ok' : 'dif-mal')}>{money(d.neto)}</td>
                <td className="num"><b>{money(d.acumulado)}</b></td>
              </tr>
            ))}
            {data && data.dias.length === 0 && <tr><td colSpan={8} style={{ textAlign: 'center', color: 'var(--faint)', padding: 24 }}>Sin movimientos de dinero en este periodo.</td></tr>}
          </tbody>
        </table>
      </div>
      <p className="faint" style={{ marginTop: 10 }}>Entradas: ventas de todos los medios y entradas manuales de caja. Salidas: compras y gastos pagados con caja menor, Nequi o Bold. Pasar el efectivo a la caja menor al cerrar no es una salida: la plata sigue en el negocio hasta que se paga algo con ella.</p>
    </>
  )
}
