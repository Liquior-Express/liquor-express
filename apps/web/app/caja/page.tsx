...
interface Actual {
  ...
  sesion?: { id: string; apertura: string; base_apertura: number; base_reales: number; abierta_por: string | null }
  resumen?: Resumen
}
interface Sesion {
  id: string; apertura: string; cierre: string; total_ventas: number; esperado_caja: number; contado_efectivo: number
...
const fecha = (s: string) => new Date(s).toLocaleString('es-CO', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })
...
function Caja() {
  ...
  const [base, setBase] = useState({ pesos: '', reales: '' })
  ...
  const cargar = useCallback(async () => {
    ...
    if (gestor) setHistorial((await apiFetch<{ sesiones: Sesion[] }>('/api/caja/historial')).sesiones)
  }, [gestor])
  useEffect(() => {
  ...
  async function abrir(e: React.FormEvent) {
    ...
    try {
      await apiFetch('/api/caja/abrir', { method: 'POST', body: JSON.stringify({ base: Number(base.pesos) || 0, base_reales: Number(base.reales) || 0 }) })
      setBase({ pesos: '', reales: '' }); setUltimoCierre(null); await cargar()
    ...
    <div style={{ maxWidth: 900 }}>
      ...
      {ultimoCierre && (
        <div className="card" style={{ maxWidth: 'none', marginBottom: 18 }}>
          <div className="row-between"><h3 className="sub-modal" style={{ margin: 0 }}>Cierre de caja registrado</h3>
            <button className="link-btn" onClick={() => setUltimoCierre(null)}>✕</button></div>
          <div className="tiles">
      ...
      {!actual.abierta ? (
        <form className="card" onSubmit={abrir} style={{ maxWidth: 440 }}>
        ...
        <>
          <div className="row-between" style={{ flexWrap: 'wrap', gap: 10 }}>
            <span className="muted">Caja abierta desde las <b>{hora(actual.sesion.apertura)}</b>{actual.sesion.abierta_por && <> por {actual.sesion.abierta_por}</>} · base {money(actual.sesion.base_apertura)}{actual.sesion.base_reales > 0 && <> + {reales(actual.sesion.base_reales)}</>}</span>
            ...
            <div className="card" style={{ maxWidth: 'none', padding: '16px 20px' }}>
              ...
              {r.movimientos.map((m) => (
                <div key={m.id} className="row-between" style={{ fontSize: 13, padding: '4px 0' }}>
                  <span>{m.concepto} <span className="faint">· {hora(m.creado_en)}</span></span>
      ...
      {gestor && historial.length > 0 && (
        <>
          <h4 className="sub-modal" style={{ marginTop: 26 }}>Cierres anteriores</h4>
          <div className="tabla-wrap">
            <table className="tabla" style={{ minWidth: 620 }}>
              <thead><tr><th>Cierre</th><th className="num">Ventas</th><th className="num">Efectivo esperado</th><th className="num">Contado</th><th>Diferencia</th><th>Reales</th></tr></thead>
              <tbody>
                {historial.map((s) => (
                  <tr key={s.id} style={{ cursor: 'default' }} title={s.observaciones ?? ''}>
                    <td>{fecha(s.cierre)}</td>
      ...
      <Modal open={cerrando} title="Cerrar caja" onClose={() => setCerrando(false)} ancho={460}>
        {r && (
          <form className="form" style={{ marginTop: 0 }} onSubmit={cerrar}>
            <p className="muted">Cuenta el dinero del cajón y escríbelo. El sistema calcula si cuadra.</p>
